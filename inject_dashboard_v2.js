...
// 1. Update AdminTab type
if (!content.includes("'members'")) {
  content = content.replace(
    "type AdminTab = 'overview' | 'pages' | 'contacts' | 'volunteers' | 'donations' | 'settings' | 'profile' | 'governance';",
    "type AdminTab = 'overview' | 'pages' | 'contacts' | 'volunteers' | 'donations' | 'settings' | 'profile' | 'governance' | 'members';"
  );
}

// 2. Destructure contexts from useData()
...
// 3. Add forms state and handlers
...
const sidebarLink = `
              <li className="nav-item">
                <a href="#" className={\`nav-link \${adminTab === 'members' ? 'active' : 'text-dark'}\`} onClick={(e) => { e.preventDefault(); setAdminTab('members'); }}>
                  <i className="fas fa-users-cog me-2 text-muted"></i> Members Directory
                </a>
              </li>
`;
if (!content.includes("setAdminTab('members')")) {
  // Inject right after governance link
  content = content.replace(
    /<li className="nav-item">\s*<a href="#" className=\{`nav-link \$\{adminTab === 'governance' \? 'active' : 'text-dark'\}`\} onClick=\{\(e\) => \{ e\.preventDefault\(\); setAdminTab\('governance'\); \}\}>\s*<i className="fas fa-gavel me-2 text-muted"><\/i> Governance Directory\s*<\/a>\s*<\/li>/,
    "$&" + sidebarLink
  );
}

// 5. Add the Members UI Block
const membersUI = `
          {adminTab === 'members' && (
...
`;
if (!content.includes('adminTab === \'members\'')) {
  const index = content.indexOf('{adminTab === \'settings\' && (');
  if (index !== -1) {
    content = content.slice(0, index) + membersUI + '\n          ' + content.slice(index);
  } else {
    console.error("Could not find settings UI block");
  }
}
